...
const form = ref({ code: '', nombre: '', is_active: true });
...
async function loadData() {
  loading.value = true;
  try {
    const catResult = await fetchApi<{ results: Category[]; count: number }>(`/api/v1/categories/?page=${page.value}&page_size=${pageSize.value}`);
...
function openCreate() {
  form.value = { code: '', nombre: '', is_active: true };
