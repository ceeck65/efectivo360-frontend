        ...
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-50/50 dark:bg-[#1a1f2e]/50">
            <tr>
              ...
              <th class="px-6 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider dark:text-slate-400">Último acceso</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-white/[0.06]">
            ...
            <tr v-for="u in filteredUsers" :key="u.id" class="hover:bg-slate-50/30 dark:hover:bg-[#1a1f2e]/30 transition-colors">
              ...
              <td class="px-6 py-4">
                <span v-if="u.last_login" class="text-sm text-slate-600 dark:text-slate-400">
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option v-for="role in tenantRoleOptions" :key="role.value" :value="role.value">
...
/**
 * @fileoverview Vista de gestión de usuarios de Tenant
 * @module @modules/users/views/TenantUsersView
 */
import { ref, computed, onMounted } from 'vue';
import { useAuthStore } from '@/stores/auth';
import { useApi } from '@/composables/useApi';
import { useNotify } from '@/composables/useNotify';
import type { User, UserRole, RoleOption } from '../types';
import { Plus, Users, Search, X, Check, Pencil, Trash2 } from 'lucide-vue-next';
...
const tenantRoleOptions = computed(() => TENANT_ROLE_OPTIONS);
...
const loadUsers = async () => {
  try {
    ...
    const allUsers = (data as { results?: User[] }).results ?? (data as User[]) ?? [];
    
    // Filter only tenant users (not staff)
    users.value = allUsers.filter(u => !u.is_staff && 
      (u.role === 'OWNER' || u.role === 'MANAGER' || u.role === 'SALES_CLERK' || u.role === 'WAREHOUSE')
    );
  } catch (error) {
...
const getRoleLabel = (role: UserRole) => {
  const option = TENANT_ROLE_OPTIONS.find((r) => r.value === role);
...
const handleDelete = async (userId: string, role: UserRole) => {
  if (role === 'OWNER') {
    notifyError('No se puede eliminar al dueño de la tienda');
    return;
  }
...
onMounted(() => {
  if (!canManageUsers.value) {
    notifyError('No tiene permisos para gestionar usuarios');
    return;
  }
  loadUsers();
});
