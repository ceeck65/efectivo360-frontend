    ...
    <div class="w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-white/[0.06] dark:bg-[#141824]">
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          <input
            v-model="form.required_features_str"
            type="text"
        ...
        <div>
          ...
          <TagsInput
            ...
            placeholder="Escribe categoría y presiona Enter..."
          />
        </div>
...
/**
 * @fileoverview Modal para editar blueprint
 * @module @modules/blueprints/components/EditBlueprintModal
 */
import { ref, watch } from 'vue';
import { useApi } from '@/composables/useApi';
import { useNotify } from '@/composables/useNotify';
import type { Blueprint, BusinessConditions, ProductSchemaField } from '../types';
...
watch(() => props.blueprint, (blueprint) => {
  if (blueprint && props.isOpen) {
    form.value = {
      ...
      required_features_str: (blueprint.required_features || []).join(', '),
      default_categories: blueprint.default_categories || [],
      business_conditions: {
        fractional_sale: false,
        service_management: false,
        attribute_control: false,
        traceability: false,
        ...blueprint.business_conditions,
      },
      product_schema: blueprint.product_schema || [],
    };
  }
});
...
const handleSubmit = async () => {
  ...
  const requiredFeatures = (form.value.required_features_str || '')
    .split(',')
  ...
  try {
    saving.value = true;
    await fetchApi(`/api/v1/industry-blueprints/${props.blueprint.id}/`, {
      method: 'PATCH',
      data: {
