...
function mapProduct(item: any): Product {
  ...
  return {
    ...
    currentStock: Number(item.available_stock ?? item.total_stock ?? item.current_stock ?? item.base_unit_stock ?? 0),
    ...
    availableStock: Number(item.available_stock ?? item.total_stock ?? 0),
    imageUrl: item.image ?? '',
    priceUsd,
    unitsPerPackage,
